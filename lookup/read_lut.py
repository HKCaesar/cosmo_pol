...
FOLDER_LUT=os.path.dirname(os.path.realpath(__file__))+'/final_lut/'

from cosmo_pol.lookup.compute_lut_sz import FREQUENCIES, HYDROM_TYPES

def get_lookup_tables(scheme, freq,load_rcs=False):
    try:
        lut_sz = pickle.load(gzip.open(FOLDER_LUT+'all_luts_SZ_f_'+str(freq).replace('.','_')+'_'+str(scheme)+'mom.pz','r'))
    except:
        raise
        print('No set of polarimetric lookup tables corresponding to this specific frequency was found '+
         'Creating a set of lookup tables by interpolating from the two closest frequencies '+
        'This might take a while (but will be done only once)')
        interpolate_lookup_tables(scheme,freq)
        return get_lookup_tables(scheme,freq,load_rcs)
    return lut_sz
