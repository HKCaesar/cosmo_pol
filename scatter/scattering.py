...
from cosmo_pol.utilities.utilities import nansum_arr, sum_arr, nan_cumprod
from cosmo_pol.utilities.utilities import  divide_by_power


LIST_OBSERVABLES = ['ZH','ZV','PHIDP','ZDR','KDP','RHOHV']
...
def get_radar_observables(list_beams, lut):
    ...
    for j,h in enumerate(hydrom_types): # Loop on hydrometeors
        ...
        for i,beam in enumerate(list_beams[0:]): # Loop on subbeams
            ...
            KDP_prof = lut[h]['KDP'].lookup_pts(lut_pts) + lut[h]['DELTAHV'].lookup_pts(lut_pts)
...
#            PHIDP_prof=nan_cumsum(KDP_prof+DELTAHV_prof)*cfg.CONFIG['radar']['radial_resolution']/1000
            ...
            rad_obs_integrated['KDP'][:,j] = nansum_arr(rad_obs_integrated['KDP'][:,j],KDP_prof*np.sqrt(beam.GH_weight))
            rad_obs_integrated['RHOHV'][:,j] = nansum_arr(rad_obs_integrated['RHOHV'][:,j],RHOHV_prof**np.sqrt(beam.GH_weight))
            ...
            sum_weights=sum_arr(sum_weights,~np.isnan(QM)*np.sqrt(beam.GH_weight))
            sum_weights_sqrt=sum_arr(np.sqrt(sum_weights),~np.isnan(QM)*np.sqrt(beam.GH_weight))
            
        # Rhohv and Kdp are divided by the total received power
        rad_obs_integrated['KDP'][:,j] = divide_by_power(rad_obs_integrated['KDP'][:,j],
                                            sum_weights_sqrt)

        rad_obs_integrated['RHOHV'][:,j] = divide_by_power(rad_obs_integrated['RHOHV'][:,j],
                                            sum_weights_sqrt)
...
def combine(rad_obs_integrated):
    ...
    # KDP
    # Just sum
    output['KDP']=np.nansum(rad_obs_integrated['KDP'],axis=1)
    # PHIDP
...
#    kdp_full[1:] = np.diff(output['PHIDP'],axis=0)
    output['PHIDP'] = np.cumsum(2 * output['KDP'],axis=0) * cfg.CONFIG['radar']['radial_resolution']/1000
