...
def ref_ODE_s(range_vec, elevation_angle, coords_radar, N):
    ...
    coords_rad_in_COSMO=pycosmo.WGS_to_COSMO(coords_radar,[proj_COSMO['Latitude_of_southern_pole'],proj_COSMO['Longitude_of_southern_pole']])
    ...
    n_vert_profile=1+(N.data[:,np.round(pos_radar_bin[0]),np.round(pos_radar_bin[0])])*1E-6
    # Get corresponding altitudes
    h=N.attributes['z-levels'][:,np.round(pos_radar_bin[0]),np.round(pos_radar_bin[0])] 
